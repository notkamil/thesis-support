...
use otterbrix_bench::scenarios::{
    CREATE_TABLE_SQL, DB, N_HEADLINE, N_WARMUP, SEED_SMALL, TBL,
};
...
fn main() {
    let workdir = fresh_workdir("s1_sqlx_");
    ...
    rt.block_on(async {
        ...
        let insert_sql = format!(
            "INSERT INTO {DB}.{TBL} (id, name, x) VALUES (?, ?, ?);"
        );
    ...
    csv::write_samples("s1_headline_sqlx", &samples);
}
